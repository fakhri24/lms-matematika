...
- **Formatif adaptif butuh headroom PER LEVEL, bukan cuma total 10.** Naik level mensyaratkan 3 benar berturut-turut (`perbaruiLevelAdaptif`), dan tiap jawaban benar BARU mengunci soal itu keluar dari pool "belum benar" secara **permanen** untuk seluruh sisa remedial siswa di sub-materi itu — `idSoalSudahBenar` direstore dari `progres_belajar` (`latihanService.js`), bukan direset per sesi. Rumus: `min soal per level = 3 + K`, dengan K = toleransi salah sebelum siswa TERPAKSA diberi soal yang jawabannya sudah dia tahu. K negatif berarti cacat struktural (bahkan tanpa satu kesalahan pun, siswa tetap dipaksa mengulang). Kebijakan yang dipakai sejak 2026-07-28: **Level 1** (lantai, tanpa penurunan di bawahnya) butuh K≥4 (min 7 soal); **Level 2 & 3** (transit, 2x salah langsung turun level) butuh K≥2 (min 5 soal). Jalankan `plan/diagnostik/gate-c-audit-bank-soal.mjs "<sub_materi>"` untuk mengecek sebelum menambah/mengurangi soal satu sub-materi. Rinciannya di [plan/PLAN.md](plan/PLAN.md) §13.2.
