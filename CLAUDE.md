...
- **Draf soal wajib lolos Gate D sebelum diimpor.** Jalankan `node plan/diagnostik/gate-d-validasi-draf-soal.mjs` (tanpa argumen = seluruh draf di `arsip-data/bank_soal/`). Impor ke Firestore satu arah — soal cacat baru ketahuan saat siswa mengerjakannya. Yang diperiksa, di luar skema: **opsi yang BERNILAI SAMA** (mis. `$4\sqrt{10}$` dengan `$2\sqrt{40}$` — soal jadi punya dua jawaban benar) dan duplikat terhadap bank yang sudah ada. Berkas yang seluruh entrinya sudah ber-`id` dilewati otomatis sebagai arsip hasil impor. Temuan bertanda `PERIKSA` bersifat peringatan, bukan penghalang.
...
- **Formatif adaptif butuh headroom PER LEVEL, bukan cuma total 10.** Naik level mensyaratkan sejumlah benar **MANDIRI** (percobaan pertama, tanpa lihat clue/pembahasan) secara **kumulatif** — bukan streak/berturut-turut, boleh diselingi salah — 4 kali untuk Level 1 & 2, 2 kali untuk Level 3 (`AMBANG_NAIK_LEVEL` di `perbaruiLevelAdaptif`, `soalEngine.js`; disamakan dengan kuota draf sumatif 4/4/2). Tiap jawaban benar (mandiri atau tidak) BARU mengunci soal itu keluar dari pool "belum benar" secara **permanen** untuk seluruh sisa remedial siswa di sub-materi itu — `idSoalSudahBenar` direstore dari `progres_belajar` (`latihanService.js`), bukan direset per sesi. Rumus: `min soal per level = benar-mandiri-untuk-naik + K`, dengan K = toleransi salah sebelum siswa TERPAKSA diberi soal yang jawabannya sudah dia tahu. K negatif berarti cacat struktural. Kebijakan yang dipakai sejak 2026-07-28: **Level 1** (lantai, tanpa penurunan di bawahnya) butuh K≥4 (min 8 soal); **Level 2** butuh K≥2 (min 6 soal); **Level 3** (ambang naik lebih rendah, 2 bukan 4) butuh K≥2 (min 4 soal). Jalankan `plan/diagnostik/gate-c-audit-bank-soal.mjs "<sub_materi>"` untuk mengecek sebelum menambah/mengurangi soal satu sub-materi. Rinciannya di [plan/PLAN.md](plan/PLAN.md) §13.2 dan §14.
- **`tingkat_kesulitan` adalah beban kognitif, bukan label urutan.** Patokannya: **soal L3 termudah harus tetap lebih berat daripada soal L1 tersulit** di sub-materi yang sama. Gate C **tidak** memeriksa ini — ia mengukur kemiripan kalimat, bukan beban. Audit lima sub-materi Eksponen (2026-08-06) menemukan **tepat satu soal L3 yang isinya setara L1 di kelima-limanya** — konsisten begitu di bank yang dibangun terpisah, jadi perlakukan sebagai cacat yang harus dicari, bukan kebetulan. Akibatnya nyata: siswa yang naik ke L3 justru mendapat soal lebih ringan, sehingga `AMBANG_NAIK_LEVEL` terpenuhi tanpa penguasaan yang sepadan. Periksa manual saat menambah/mengubah soal; rinciannya di [plan/PLAN.md](plan/PLAN.md) §18.1.
- **Pengecoh dilarang bernilai sama dengan kunci — termasuk "bentuk yang belum disederhanakan".** Menaruh $\sqrt{25}$ di samping $5$, atau $\frac{12}{\sqrt6}$ di samping $2\sqrt6$, bukan memasang pengecoh melainkan **menyediakan jawaban benar kedua**; siswa yang memilihnya bisa berargumen dan ia benar. Kalau miskonsepsinya penting, tempatnya di **`pembahasan`**, atau soalnya diubah jadi pemilihan-bentuk yang kriterianya tajam ("manakah yang penyebutnya SUDAH rasional") dengan lima opsi bernilai berbeda. Gate D menandai ini **fatal** — jangan menyiasatinya, ubah rancangan soalnya. Lima kasus ditemukan pada audit 2026-08-06, dua di antaranya melekat pada topik merasionalkan penyebut ([plan/PLAN.md](plan/PLAN.md) §18.2).
- **Cek lintas sub-materi sebelum menambahkan konsep baru ke sebuah sub-materi.** Gate C dan Gate D dua-duanya berjalan **per-sub-materi**, jadi keduanya buta terhadap duplikat hulu-hilir — dan yang paling berbahaya adalah hulu mencuri materi hilirnya, karena hilir jadi kehilangan alasan untuk ada. Sebelum menulis soal berkonsep baru, `grep` dulu seluruh `arsip-data/bank_soal_all.json`. Dua nyaris-duplikat yang tertangkap cara ini (2026-08-06): akar pangkat tiga (milik `Eksponen Rasional`) hampir masuk ke `Operasi Bentuk Akar`, dan model pertumbuhan populasi (milik `Fungsi Eksponen`) hampir masuk ke `Eksponen Rasional`. Lihat [plan/PLAN.md](plan/PLAN.md) §18.3.
